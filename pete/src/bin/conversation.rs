...
#[derive(Parser)]
#[command(author, version, about = "Print a text timeline of conversation sensations (hearing and saying)")]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    ...
    let items = graph
        .conversation_timeline(from, to, cli.limit)
        .await
        .context("failed to load conversation timeline")?;
...
fn print_timeline(
    ...
    println!("Conversation timeline {} to {}", from, localized_timestamp(to));
    if items.is_empty() {
