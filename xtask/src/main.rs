...
fn print_help() {
    ...
    println!(
        "  fetch [tiny.en|base.en|small.en|large-v3|URL]            # fetches the default fast local model, or one model if specified"
    );
    println!(
        "  fetch-asr-model [tiny.en|base.en|small.en|large-v3|URL]  # fetches ASR and voice embeddings"
    );
...
mod tests {
    ...
    #[test]
    fn model_choice_keeps_large_multilingual_alias() {
        let (url, path) = model_choice("large-v3");
