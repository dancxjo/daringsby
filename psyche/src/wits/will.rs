...
impl Will {
    ...
    /// Create a new `Will` subscribed to `bus` using `doer`.
    ...
    /// Create a new `Will` emitting [`WitReport`]s via `tx`.
    pub fn with_debug(
        bus: TopicBus,
        doer: Arc<dyn Doer>,
        tx: Option<broadcast::Sender<WitReport>>,
...
#[async_trait]
impl crate::wit::Wit<(), Instruction> for Will {
    ...
    async fn tick(&self) -> Vec<Impression<Instruction>> {
        ...
        info!(prompt = %prompt_text, "will prompt");
        let resp = match self
            .doer
