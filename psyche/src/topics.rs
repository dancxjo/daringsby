...
pub enum Topic {
    ...
    ///
    /// Generated by the Quick by summarizing a batch of timestamped
    /// [`Stimulus`](crate::Stimulus) values into an
    /// [`Impression`](crate::Impression). Represents Pete’s immediate
    /// experience (“I see a fly approaching”).
    Instant,

    /// 🧭 Integrated summary of nearby events.
    ///
    /// Emitted by the Combobulator by collecting several immediate
    /// [`Impression`](crate::Impression)s into a cohesive moment. Useful for
    /// recognizing short patterns or behavior changes (“The fly hovered near
    ...
    ///
    /// Constructed by Memory using linked [`Impression`](crate::Impression)s.
    /// Represents a durable event in Pete’s timeline.
...
/// ```rust,ignore
/// use futures::StreamExt;
/// use psyche::{topics::{Topic, TopicBus}, Impression, Stimulus};
///
/// let bus = TopicBus::new(8);
/// // A Quick emits an immediate impression.
/// let impression = Impression::new(
///     vec![Stimulus::new("User said hi".to_string())],
///     "The user greeted Pete.",
///     None::<String>,
/// );
/// bus.publish(Topic::Instant, impression);
///
/// // Another wit subscribes to those immediate impressions.
/// let mut rx = bus.subscribe(Topic::Instant);
/// while let Some(payload) = rx.next().await {
///     let _ = payload.downcast::<Impression<String>>().unwrap();
/// }
/// ```
///
