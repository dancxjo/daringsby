...
export class Psyche {
    ...
    constructor(
        ...
        this.quick = new Wit(
            ...
            (experiences) => {
                const happenings = experiences.map((s) => `[${s.what[0]?.when}] ${s.how}`).join("\n");
                return `
...
Respond with just the sentence — nothing more.`;
            },
            {
                ...
                onStream: (name, chunk) => this.opts.onStream?.("quick", chunk) ?? Promise.resolve(),
            },
        );
...
Return only the resulting memory.`;
            ...
            {
                onPrompt: (name, prompt) => this.opts.onPrompt?.("combobulator", prompt) ?? Promise.resolve(),
                onStream: (name, chunk) => this.opts.onStream?.("combobulator", chunk) ?? Promise.resolve(),
            },
        );
...
Respond with just one emoji (any single unicode emoji) — nothing more and nothing less.`;
            },
            {
                ...
                onStream: (name, chunk) => this.opts.onStream?.("heart", chunk) ?? Promise.resolve(),
            },
        );
        ...
        try {
            ...
            const reply = await this.chatter.chat(
                ...
                async (chunk) => {
                    ...
                    await this.opts.onStream?.("speech", chunk);
                },
            );
